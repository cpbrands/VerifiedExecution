...
Event {
  ...
  sequence: required non-negative per-Action ordinal
...
| `event_type` | Required; exactly one; non-null | Exact identifier of the fact asserted by the Event | Exact identifier equality; lexical order has no semantic meaning | Chosen by the Boundary only after the governing definition is resolved; interpreted by that definition and applicable projections | VE-003 for its lifecycle vocabulary; otherwise an explicit event-type profile | Historical assertion kind |
...
| `sequence` | Required; exactly one; non-null | Non-negative ordinal allocated in one Action's authoritative Event stream | Unique within the Action stream; strictly orders that stream; no cross-Action meaning | Allocated by the authoritative append mechanism; interpreted by history replay | This contract under VE-002 ordering | Historical stream order |
...
domain or execution profile MAY define additional Event types. Such a profile
MUST use an identifier that is unambiguous in its declared authority and MUST
remain historically resolvable. This contract creates no global event-type
registry and no negotiation layer.

An implementation MUST NOT infer an event type from `payload`, `actor`,
`component`, observations, a Receipt, current state, or an identifier naming
convention. A type name defined by two authorities is not portable until the
applicable authority is unambiguous.
...
`sequence` is a mathematical non-negative integer assigned within exactly one
Action's authoritative Event stream. Its rules are:
...
execution. A concrete representation profile MUST define the finite integer
range and encoding without changing these semantics.
...
event-type definition is selected by the deployment's governing semantic
context from the exact `event_type` and declared authority, not by silently
changing the VE-002 version field.
...
- a Policy or Rule artifact, interpreted by its owning specification;
...
- it SHOULD be preserved when lossless forwarding or archival is claimed.
...
5. allocate the Event occurrence identity and per-Action sequence under their
   governing rules;
...
- an optional top-level field is present with null;
...
- its `event_type` cannot be resolved under the governing context;
...
| N3 | Null optional top-level field | reject as nonconforming |
...
| N10 | Unknown event type under resolved governing context | unsupported for interpretation; do not project |
...
| Primitive burden | **PASS.** It closes fields on the existing Event primitive and introduces no new primitive, registry, identity, clock, or reference object. |
| Removability | **PASS.** Profile-specific actor, component, payload, reference, and time details can be omitted when an event type does not require them; the base Event architecture remains intact. |
| Twenty-year durability | **PASS.** Durable interpretation requires exact historically resolvable VE-002, event-type, and subordinate profile authority; mutable deployment guesses cannot rewrite history. |
| Independent implementability | **PASS.** Required fields, ownership, sequence behavior, null/absence/default rules, profile duties, authority, and failure behavior are explicit before encoding. |
| Reduced conceptual complexity | **PASS.** One companion contract assigns existing field ownership and leaves varying domain semantics to explicit profiles without universal registries or schemas. |
