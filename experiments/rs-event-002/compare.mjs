...
export function provenance(){
  const catalog={};
  for(const p of fixtures.sources){const bytes=historical(p);verifyBytes(p,bytes);catalog[p.path]=bytes.toString('utf8');}
  return catalog;
}
...
export function validateSuite(f=fixtures,o=oracle){
...
export function inputFor(c,catalog){
  const i=expand(c.input);
...
export async function runComparison(){
  validateSuite();const catalog=provenance();
  const {evaluate}=await import('./rule-graph.mjs');
  const counts={};
  for(const [index,c]of fixtures.cases.entries()){
    try {
      const i=inputFor(c,catalog), a=await invoke(i), b=evaluate(structuredClone(i));
      comparePair(a,b,oracle.rows[index]);
      counts[a.classification]=(counts[a.classification]??0)+1;
    } catch(error) {throw Error(`case ${c.id}: ${error.message}`,{cause:error});}
  }
  return {cases:87,counts,sourcePins:fixtures.sources.length};
}
