...
`action_id` selects the historical Action occurrence. `action_digest` binds
the assertion to the exact semantic content accepted for that occurrence.
...
particular repeated Action request.

This binding does not identify an individual execution attempt. Approved
VE-001 permits multiple execution attempts for one Action occurrence.
Attempt-specific telemetry must bind through an Event/history reference or
through future accepted attempt semantics. This result does not introduce
an `attempt_id` primitive.
...
Issuer is required Claim-body semantics, represented as an externally
resolvable `issuer_ref`. It is not merely signature or verification
metadata.

Bank A and Bank B can make the same abstract proposition about Account X,
while producing distinct VE Claims because issuer affects trust and
evaluation semantics. Removing issuer from the Claim body would make
independently verified identical assertion content ambiguous: separate
issuers could assert the same predicate and value, while a verifier could
not express which assertion it was evaluating. A signing key alone is
insufficient because key material may be delegated, rotated, or used on
behalf of a distinct issuer. Trust, delegation, revocation, and
multi-issuer evaluation therefore need the assertion's claimed issuer to
be explicit; verification evidence determines whether that issuer assertion
is valid in the applicable Trust Context.

`issuer_ref` is not signer identity, a verification key, a DID, a
certificate subject, or VE-managed identity. The reference does not make VE
an identity provider. VE need not define a person, organization, account,
or identity registry. It need only preserve an external reference whose
resolution and trust meaning are governed outside the minimal Claim
structure.
...
|---|---|---|
| `assertion_time` | OPTIONAL, PROVISIONAL | It is the time the issuer made the assertion. It may matter to validity, replay, or policy, but not every Claim requires it. |
| `observation_time` | OPTIONAL, PROVISIONAL | It is a domain fact about when the observed condition occurred. It is distinct from assertion time and is required only when the applicable schema/profile makes it material. |
...
When either time is present, it changes the canonical Claim body. Its
absence means “not asserted / not material.” Presence and absence are not
interchangeable representations of the same Claim. The exact canonical
presence and omission rules remain specification work.
...
| Action occurrence | `kind: action-occurrence` plus `action_id` and `action_digest`, or an accepted equivalent content reference | A Claim about one historical Action occurrence. |
| Execution attempt | Event/history reference, or future accepted attempt semantics | A telemetry assertion about one attempt under an Action occurrence. |
...
The attempt form answers “which execution attempt, if attempt-specific
meaning is required?” That structural distinction prevents a Claim about
one Action occurrence from being reused as evidence for another occurrence
of the same Action content, without silently treating occurrence identity
as attempt identity.

## Provisional Claim structure
...
The six semantic fields are a candidate for `Claim.body`, not the complete
Claim. `verification { profile, artifact }` is required by accepted
ADR-VERIFY-002 for portable Claim verification.
...
| `issuer_ref` | REQUIRED | Identifies the semantic issuer; verification establishes whether that issuer assertion is trustworthy. |
...
| `assertion_time` | OPTIONAL, PROVISIONAL | Records when the issuer made the assertion when material; presence changes the canonical Claim body. |
| `observation_time` | OPTIONAL, PROVISIONAL | Records when the observed condition occurred when material to the domain; presence changes the canonical Claim body. |
...
format beyond the accepted verification envelope, Trust Context, Rule input
mapping, or receipt derivation. Those remain separate responsibilities.
...
portable verification artifact under its Trust Context. Optional assertion
...
identity authority is demonstrated. No RFC is required for this pressure-test
correction itself. Normative adoption must follow normal governance if it
changes an Approved specification. RFC-005 remains Draft and non-blocking:
any future portable encoding work for structured references must not be
...
subject references. It must explicitly distinguish Action content reference,
Action occurrence reference, and attempt/history reference. It must not
invent attempt identity if Event/history semantics already solve the
attempt-specific binding. It must not create an Observation Claim subtype,
Action Execution Profile, new authority model, or new primitive.
...
**C. INCONCLUSIVE — DEPENDENCY UNRESOLVED**
